...
% Now draw the chips on the board. We find piece locations from checkers.logicBoard,
% then paint them on to the corresponding positions in imageBoard.
for r = 1:8
    for c = 1:8
    switch checkers.logicBoard(r,c)
        % Draw our pawns
        case {1, 2} 
            ...
            tile(mask) = checkers.logicBoard(r,c);
        ...
        case {11, 22} 
            tile = imageBoard(1+(r-1)*res/8:r*res/8, 1+(c-1)*res/8:c*res/8);
            tile(mask) = checkers.logicBoard(r,c)/11;
            tile(kingMask) = 5;
            imageBoard(1+(r-1)*res/8:r*res/8, 1+(c-1)*res/8:c*res/8) = tile;  
    end
    end
