...
% write capture chain moves to checkers.moveList.
%
...
switch player
    case true % For player 1
        ...
        if ((hopBoard(ul) == 2) || (hopBoard(ul) == 22)) && ...
            ...
            if hopBoard(ul+checkers.offsets.ul) == 0
                
                % Take the capture and see if we can cap again (recursive)
                moveWIP = [move, ul, ul+checkers.offsets.ul];
                evalHop(ul+checkers.offsets.ul, moveWIP)
        ...
        if ((hopBoard(ur) == 2) || (hopBoard(ur) == 22)) &&...
            ...
            try
            if hopBoard(ur+checkers.offsets.ur) == 0
                
                moveWIP = [move, ur, ur+checkers.offsets.ur];
                evalHop(ur+checkers.offsets.ur, moveWIP)
        ...
        % move sequence to checkers.moveList and go home. However, only do this for
        % pawns. Kings need to fail 4 times. move(1) holds the index of the
        % playing piece.
        if fail == 2 && hopBoard(move(1)) == 1
            checkers.moveList{end+1} = move;
            checkers.moveList{1} = [checkers.moveList{1}, length(checkers.moveList)];
        end
        ...
        if ((hopBoard(bl) == 2) || (hopBoard(bl) == 22)) &&...
            mod(i,8) < 7
            try
            if hopBoard(bl+checkers.offsets.bl) == 0
                moveWIP = [move, bl, bl+checkers.offsets.bl];
                evalHop(bl+checkers.offsets.bl, moveWIP)
            else
                fail = fail+1;
            end
            catch
        ...
        if ((hopBoard(br) == 2) || (hopBoard(br) == 22)) &&...
            mod(i,8) < 7
            try
            if hopBoard(br+checkers.offsets.br) == 0
                moveWIP = [move, br, br+checkers.offsets.br];
                evalHop(br+checkers.offsets.br, moveWIP)
            else
                fail = fail+1;
            end
            catch
